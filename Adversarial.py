...
def main():
    for noise_type in noise_types:
        ...
        if noise_type==0:
            ...
            parser.add_argument('--epsilon', default=8/255, type=float, help='epsilon')
            parser.add_argument('--alpha', default=2/255, type=float, help='alpha')
            parser.add_argument('--num_iter', default=1, type=int, help='num_iter')

            parser.add_argument('--noiseConfig', default=NoNoiseConfig(), type=bool, help='no noise')
            args = parser.parse_args()

            solver = AdversarialSolver(args)
            acc = solver.run()
            accuracies.append({"noise_type":noise_type,"acc":acc,"epochs":epochs})

            with open(f"adversarial_results_{noise_type}_{timestamp}.json", "w") as f:
        ...
        if noise_type==1:
            ...
            for gauss_std in gauss_stds:
                ...
                solver = AdversarialSolver(args)
                acc,acc_adv = solver.run()
                accuracies.append({"noise_type":noise_type,"gauss_std":gauss_std,"acc":acc,"acc_adv":acc_adv,"epochs":epochs})
                with open(f"adversarial_results_{noise_type}_{timestamp}.json", "w") as f:
                    json.dump(accuracies, f)
...
class AdversarialSolver(object):
    ...
    def load_model(self):
        ...
        self.model = LeNet(self.noise_config,self.epsilon,self.alpha,self.num_iter).to(self.device)
    ...
    def train(self):
        ...
        for batch_num, (data, target) in enumerate(self.train_loader):
            ...
            x_adv = self.model.pgd_attack(data, target)
    ...
    def run(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            test_result_adv = self.test_adv()
            accuracy_adv = max(accuracy_adv, test_result_adv[1])

            if epoch == self.epochs:
                print("===> BEST ACC. PERFORMANCE on clean data: %.3f%%" % (accuracy * 100))
                print("===> BEST ACC. PERFORMANCE on adversarial data: %.3f%%" % (accuracy_adv * 100))
                self.save()
