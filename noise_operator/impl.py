...
class CombinedGaussianNoise(object):
    ...
    def __call__(self, tensor):
        ...
        if self.FirstMulThenAdd:
            out = tensor * torch.normal(self.GaussMeanMul, self.GaussStdMul, tensor.size(), device=device)
            out += torch.normal(self.GaussMeanAdd, self.GaussStdAdd, tensor.size(), device=device)
        else:
            out = tensor + torch.normal(self.GaussMeanAdd, self.GaussStdAdd, tensor.size(), device=device)
            out *= torch.normal(self.GaussMeanMul, self.GaussStdMul, tensor.size(), device=device)
        return out
